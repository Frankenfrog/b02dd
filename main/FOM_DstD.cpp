...
int main(int argc, char * argv[]){
  ...
  RooRealVar        obsMass("B0_LOKI_DTF_MASS_3D","#it{m_{D^{+} D^{-}}}",5150,5500,"MeV/c^{2}");
  RooRealVar        obsTime("B0_FitPVConst_tau_flat","#it{t}",0.,0.01025,"ps");
  ...
  RooRealVar        obsTimeErr("B0_FitPVConst_tauErr_flat","#sigma_{t}",0.000005,0.0002,"ps");
  ...
  RooRealVar        BDT_classifier(TString(config.getString("bdt_classifier")),"BDT_classifier",-1,1);
...
//=========================================================================================================================================================================================================================
  ...
  if (fits) {
      ...
      cout << cutvalue  << endl;
      reduced_data = dynamic_cast<RooDataSet*>(data->reduce(TString(config.getString("bdt_classifier")+">"+to_string(cutvalue))));
      pdfMass->getParameters(data)->readFromFile("/home/fmeier/git/b02dd/config/StartingValues/StartingValues_Mass.txt");
      reduced_data->Print();
      ...
      for (int j = 0; j < reduced_data_sweighted->numEntries(); ++j) {
        ...
        resolution_dilution_exp = exp(-pow(dmd*1000*reduced_data_sweighted->get()->getRealValue("B0_FitPVConst_tauErr_flat"),2));
        resolution_dilution_value += weight*resolution_dilution_exp;
        time = 1000*reduced_data_sweighted->get()->getRealValue("B0_FitPVConst_tau_flat");
    ...
    mg->SetMinimum(0.5);
    mg->SetMaximum(1.1);
    c.BuildLegend(0.45,0.2,0.7,0.5);
    printPlot(&c, "Summary", TString("/home/fmeier/lhcb-tank/b02dd/run/FOM"));	
  }
