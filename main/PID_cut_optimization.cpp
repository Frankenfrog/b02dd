...
int main(int argc, char * argv[]){
  ...
  RooRealVar        BDT2_classifier("BDT2_classifier","BDT2_classifier",0,1);
  ...
  // Get data set
  EasyTuple         tuple("/fhgfs/groups/e5/lhcb/NTuples/B02DD/Data/Combined_2011_2012/DT20112012_B02DD_Stripping21r0r1_DVv36r1_20150322_fmeier_combined_20150412_fmeier_BDT_TupleB_relevantfinalstates.root","B02DD",RooArgSet(realvars,categories));
  ...
  // for (int i = 0; i < 1; ++i) {
  //   double cutvalue = -1+0.01*i;
  //   cfg_plot_mass.set_plot_appendix("_"+to_string(i));
  //   reduced_data = dynamic_cast<RooDataSet*>(data.reduce(TString("BDT2_classifier>"+to_string(cutvalue))));
  //   reduced_data->Print();
  //   Plot Mass(cfg_plot_mass, obsMass, *reduced_data, RooArgList(), "obsMass_with_BDT2>"+to_string(cutvalue));
  //   Mass.PlotIt();
  // }
