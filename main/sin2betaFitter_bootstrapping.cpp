  ...
  if (method.EqualTo("g") || method.EqualTo("e")) {
    ...
    if (method.EqualTo("g")) {
      ...
      for (int i = 0; i < 500 ; ++i) {
        cout  <<  i <<  endl;
        try {
          data_temp = new RooDataSet("data_temp","data_temp",RooArgSet(observables,categories));
          for (int i = 0; i < data->numEntries(); ++i) {
            data->get(random.Rndm()*data->numEntries());
            data_temp->add(*(data->get()));
          }
          data_bootstrapped = new RooDataSet("data_bootstrapped","data_bootstrapped",data_temp,RooArgSet(observables,categories),"","SigWeight");
          // EasyTuple tuple_bootstrapped(*data_bootstrapped);
          // tuple_bootstrapped.WriteDataSetToTree("/home/fmeier/storage03/Tuple/Bootstrapping_Test.root","B02DD");
          // data_bootstrapped->Print();
          pdf.getParameters(data)->readFromFile("/home/fmeier/storage03/b02dd/Bootstrapping/generation.par");
          ...
          while ((constrainingPdf = dynamic_cast<RooAbsPdf*>(iterator->Next()))){
            constrainingPdf->getParameters(*data)->readFromFile("/home/fmeier/storage03/b02dd/Bootstrapping/generation.par");
          }
          stopwatch.Start(true);
          RooFitResult* fit_result = pdf.fitTo(*data_bootstrapped,fitting_args);
          stopwatch.Stop();
          fit_result->Print("v");
